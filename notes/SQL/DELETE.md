Para que el ejemplo del DELETE funcione, primero necesitamos una tabla y algunos datos. Imaginemos una tabla llamada `Usuarios`.
...
```
> **¡¡¡ADVERTENCIA!!!** Si olvidas la cláusula `WHERE`, ¡borrarás **TODOS** los registros de la tabla!

**Ejemplo:** Eliminar al usuario con `id = 1`.
```sql
...
```
**Resultado esperado (el usuario con id=1 ya no existe):**
